...
for epoch in range(1, num_epochs + 1):
    ...
    if (len(val_losses) > 0) and (val_loss < min(val_losses)):
        torch.save(model.state_dict(), "model_{}_{:.4f}_NB.pt".format(epoch, val_loss))
...
np.save("train_history_NB", train_history)
np.save("val_history_NB", val_history)
np.save("mAP_history_NB", mAP_history)